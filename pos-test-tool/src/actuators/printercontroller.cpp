...
PrinterController::PrinterController() {
  ...
  if (core.initSerial(serialPort, QSerialPort::Baud115200) == true) {
    core.home();
    connect(&core, &AtCore::stateChanged, this,
            [this](AtCore::STATES newState) {
...
void PrinterController::setXYZPosition(qreal x, qreal y, qreal z) {
  ...
  core.pushCommand(GCode::toCommand(
      GCode::G0,
...
void PrinterController::setXYPosition(QPointF point) {
  core.setAbsolutePosition();
  core.pushCommand(GCode::toCommand(
      GCode::G0, QStringLiteral("X%1 Y%2").arg(QString::number(point.x()),
...
void PrinterController::moveX(qreal x) {
  core.setAbsolutePosition();
  core.pushCommand(GCode::toCommand(
      GCode::G0, QStringLiteral("%1%2").arg("X", QString::number(x))));
}

void PrinterController::moveY(qreal y) {
  core.setAbsolutePosition();
  core.pushCommand(GCode::toCommand(
      GCode::G0, QStringLiteral("%1%2").arg("Y", QString::number(y))));
}

void PrinterController::moveZ(qreal z) {
  core.setAbsolutePosition();
  core.pushCommand(GCode::toCommand(
      GCode::G0, QStringLiteral("%1%2").arg("Z", QString::number(z))));
}

void PrinterController::wait(int millisseconds) {
  core.pushCommand(
      QStringLiteral("G4 P%1").arg(QString::number(millisseconds)));
}
